...
Welcome To Sipun's Safeum Tool  

-------------------------------
""")

# Automatically open youtube channel
os.system("am start -a android.intent.action.VIEW -d https://www.facebook.com/sipunidiot")
...
def work():
    ...
    username = choice('abcdefghijklmopnqrstyuzyx') + ''.join(choices(list('abcdefghijklmopnqrstyuzyx1234567890'), k=12))
    try:
        ...
        if '"status":"Success"' in gzip:
            success += 1
            accounts.append(username + f':{G}Creator (IDIOT){R}')
            with open('Sipun.Safeum.Acc.txt', 'a') as f: 
                f.write(username + f":{G}Creator (Sipun){R}\n")
        else:
            failed += 1
    except:
        retry += 1
